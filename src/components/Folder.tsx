...
const Folder: React.FC<FolderProps> = ({ color = '#5227FF', size = 1, items = [], className = '' }) => {
  ...
  const handlePaperClick = (item: FolderItem, e: React.MouseEvent) => {
    if (open && item.image) {
      e.stopPropagation();
            ...
            return (
              <div
                ...
                className={`absolute z-20 bottom-[10%] left-1/2 transition-all duration-300 ease-in-out ${
                  !open ? 'transform -translate-x-1/2 translate-y-[10%] group-hover:translate-y-0' : 'hover:scale-110 cursor-pointer'
                } ${sizeClasses} ${open && item.image ? 'hover:ring-2 hover:ring-green-400' : ''}`}
                style={{
              ...
              >
                {item.image && open && (
                  <div className="w-full h-full rounded-lg overflow-hidden flex items-center justify-center bg-white/10 backdrop-blur-sm">
                    <img 
                      src={item.image} 
                      alt={item.title}
